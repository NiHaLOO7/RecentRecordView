...
import utils from './multiselectComboboxHelper';
const SEPERATOR = ';',
...
uniqueId = utils.uniqueStrGenerator(); // generate this Id to use in the attribute mapping
...
@track comboboxObj = {
    ...
    lazySearch: false,
    isLoading: false,
    validation: utils.deepClone(VALIDATION),
    ...
    set value(val) {
        ...
        if (!Array.isArray(val) || (Array.isArray(val) && val.length > 0)) {
            if (this._selectedValues.length > 0) {
                if (this.multiselect && !Array.isArray(val)) {
                    ...
                    valueList = utils.deepClone(this._selectedValues);
        ...
        if (!utils.compareArrayObject(valueList, this._selectedValues)) { // if the values are not same
...
set validation(value){
    if(typeof value === 'object' && value.length > 0){
        let rules = utils.deepClone(value);
...
set options(value){
    if(typeof value === 'object' && value.length > 0){
        this.comboboxObj.options = this.prepareDropdownOptionList(utils.deepClone(value));
        this.comboboxObj.dropdownList =  this.prepareDropdownOptionList(utils.deepClone(value));
    }
    else{
...
// @api methods 
    ...
    @api processSearchResult(data) {
        this.comboboxObj.options = this.prepareDropdownOptionList(utils.deepClone(data));
        this.comboboxObj.dropdownList = this.prepareDropdownOptionList(utils.deepClone(data));
        // reset the loading
...
// internal component logic
    ...
    handleSelectInputBlur(event){
        ...
        this.runValidationRules();
        utils.openAndCloseDropdown('close',this);
    }
    
    handleOptionClick(event){
        event?.stopPropagation();
        event?.preventDefault();
        utils.handleOptionClickAction(event,this);
    }
    
    handleOptionHover(event){
        event?.stopPropagation();
        event?.preventDefault();
        utils.handleOptionHoverAction(event,this);
    }
    ...
    handleInputKeyDown(event){
        ...
        utils.handleKeyDownOnInput(event,this);
    }
    ...
    handleInputKeyUp(event){
        ...
        utils.handleKeyUpOnInput(event,this);
    }
    ...
    handleInputClick(event){
        ...
        utils.handleDropdownOnInputClick(event,this);
    }
    ...
    handleClearAction(event){
        ...
        if(this.comboboxObj.autocomplete){
            utils.clearSelectionOnAutocomplete(this);
        }
    }
    ...
    handleSelectionRemoval(event){
        utils.handleSelectionRemovalAction(event,this);
    }
    ...
    prepareDropdownOptionList(options){
        ...
        for(let obj of options){
            ...
            Object.defineProperty(obj, '_id', {
                value: utils.uniqueStrGenerator()
            });
    ...
    customValidationRuleObj(dataObj){
        let ruleObj = {};
        ruleObj.name = ('name' in dataObj) ? dataObj.name ? dataObj.name : utils.uniqueStrGenerator() : utils.uniqueStrGenerator();
    ...
    runValidationRules(){
        ...
        if(errorObj && errorObj.length > 0){
            ...
            for (const error of filteredObj){
                if(utils.evaluateExpression(value,error.condition)){
